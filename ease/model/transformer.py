...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, nhead=8, d_model=256, temperature=10000, pos_type="fourier", attn_mask_thresh=0.5, num_query=400, num_class=18):
        super().__init__()
        ...
        self.tmp_cls = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, self.num_class + 1))

        self.text_embed_head = nn.Sequential(
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 256),
        )
        ...
        self.labels = (
            torch.arange(self.num_class+1, device='cuda')
            .unsqueeze(0)
            .repeat(self.num_query, 1)
            .flatten(0, 1)
        )
        
        self.query_embed_head = nn.Sequential(
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 256),
        )
    ...
    def forward(self, tgt, memory, input_ranges, coords_float, mask_feats_batched, lengths,
                ...
                instance_text_embeddings: Optional[Tensor] = None):
        output = tgt
        attn_masks = None
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            pred_labels = self.tmp_cls(output_perm)

            text_embeddings = list()
            for b_labels in pred_labels:
                # b_labels : (400, 19)
                scores_per_query, topk_indices = b_labels.flatten(0, 1).topk(self.num_query, sorted=True)
                labels_per_query = self.labels[topk_indices] # (400,)
                text_embeddings_per_query = instance_text_embeddings[labels_per_query] # (400, 512)
                text_embeddings.append(text_embeddings_per_query)
            text_embeddings = self.text_embed_head(torch.stack(text_embeddings).float()) # (bsz, 400, 256)
            text_embeddins_norm = self.text_norm(text_embeddings)
            text_embed_query = torch.cat((output_perm, text_embeddings), dim=2)
            new_query = self.query_embed_head(text_embed_query) # (bsz, 400, 256) 
            output = new_query.permute(1, 0, 2) # (400, bsz, 256)
