...
class QueryDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.labels = (
            torch.arange(num_class+1, device='cuda')
            .unsqueeze(0)
    ...
    def forward_iter_pred(self, x, pos, batch_offsets, instance_text_embeddings):
        """
        x [B*M, inchannel]
        """
        ...
        intermediate_results, ref_points = self.decoder(tgt=query.new_zeros(num_queries, B, d_model),
            ...
            instance_text_embeddings=instance_text_embeddings)

        # intermediate_results : results of each six layers 
    ...
    def forward(self, x, pos, batch_offsets, instance_text_embeddings=None):
        return self.forward_iter_pred(x, pos, batch_offsets, instance_text_embeddings)