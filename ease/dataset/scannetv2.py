...
class ScanNetDataset(Dataset):
    ...
    def transform_train(self, xyz, rgb, superpoint, semantic_label, instance_label, normal=None, boundaries=None):
        ...
        if boundaries is not None:
            boundaries = boundaries[valid_idxs]
        return xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, normal, boundaries

    def transform_test(self, xyz, rgb, superpoint, semantic_label=None, instance_label=None, normal=None, boundaries=None):
        xyz_middle = xyz
        xyz = xyz_middle * self.voxel_cfg.scale
        xyz -= xyz.min(0)
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        boundary_path = osp.join(self.data_root, self.prefix+'_boundaries', scan_id[5:]+'.npy')
        boundaries = np.load(boundary_path)
        boundaries = torch.from_numpy(boundaries)

        data = self.transform_train(*data, boundaries=boundaries) if self.training else self.transform_test(*data, boundaries=boundaries)
        xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, normal, boundaries = data
        ...
        return scan_id, coord, coord_float, feat, superpoint, inst, normal, boundaries

    def collate_fn(self, batch: Sequence[Sequence]) -> Dict:
        scan_ids, coords, coords_float, feats, superpoints, insts, boundaries = [], [], [], [], [], [], []
        batch_offsets = [0]
        superpoint_bias = 0
        ...
        for i, data in enumerate(batch):
            scan_id, coord, coord_float, feat, superpoint, inst, normal, boundary = data
        ...
        boundaries = torch.cat(boundaries, 0).float()
