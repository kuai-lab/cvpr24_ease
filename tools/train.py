...
from ease.dataset import build_dataloader, build_dataset
from ease.evaluation import ScanNetEval
from ease.utils import AverageMeter, get_root_logger, rle_decode, write_obj
import numpy as np
...
def get_args():
    ...
    parser.add_argument('--project_name', default='EASE', type=str, help='project name')
    parser.add_argument('--experiment_name', type=str, help='experiment name')
    parser.add_argument('--edge_weight', type=float, help='edge loss weight')
    parser.add_argument('--edge_lambda', type=float, help='edge loss lambda')
...
def get_model(cfg, model_name, edge_weight, edge_lambda):
    if model_name == 'EASE':
        from ease.model import EASE
        model = EASE(**cfg.model, edge_weight=edge_weight, edge_lambda=edge_lambda).cuda()
    else:
...
def main():
    ...
    if args.work_dir:
        ...
        cfg.work_dir = osp.join('./saved', args.experiment_name)
    ...
    if args.seed is not None:
        gorilla.set_random_seed(args.seed)
    elif cfg.train.seed == 0:
        rand_seed = np.random.randint(0, 1e9)
        gorilla.set_random_seed(rand_seed)
    else:
    ...
    # model
    model_name = cfg.model.pop("name", "EASE")
    model = get_model(cfg, model_name, args.edge_weight, args.edge_lambda)
